...
# Get node ID where a service is running (or 'starting' or None)
def get_service_node(service_name, wait_timeout=5):
    try:
        ...
        while time.time() < deadline:
            tasks = service.tasks()
            if not tasks:
                logging.warning(f"⚠️ No tasks found for {service_name} — service might not be scheduled")
                return None

            for task in tasks:
                status = task.get("Status", {})
                state = status.get("State")
                error_msg = status.get("Err", "").lower()
                node_id = task.get("NodeID")
                task_id = task.get("ID")

                logging.debug(
                    f"🧪 Task for {service_name}: ID={task_id} | State={state} | NodeID={node_id}"
                )

                # 1. Constraint failure (no suitable node)
                if state == "rejected" and (
                    "no suitable node" in error_msg
                    or "does not match constraints" in error_msg
                    or "could not find a node" in error_msg
                    or "unable to find node" in error_msg
                ):
                    logging.warning(f"🚫 {service_name} is rejected due to constraint failure: {error_msg}")
                    return None

                # 2. Normal running state
                if state == "running":
                    if node_id:
                        logging.info(f"📍 {service_name} is running on node {node_id}")
                        return node_id
                    else:
                        logging.warning(f"⚠️ {service_name} is running but has no node ID — possible orphan task?")
                        return None

                # 3. Still starting — could become valid soon
                if state in ["new", "allocated", "assigned", "pending", "preparing", "starting"]:
                    logging.info(f"⏳ {service_name} is in state '{state}' on node {node_id}, will retry later")
                    return "starting"

                # 4. Special case: 'ready' but no container ID
                if state == "ready":
                    container_status = status.get("ContainerStatus", {})
                    container_id = container_status.get("ContainerID")
                    if container_id:
                        logging.info(f"🧪 {service_name} is in 'ready' state with container — likely still starting")
                        return "starting"
                    else:
                        logging.warning(f"⚠️ {service_name} is stuck in 'ready' with no container. May need intervention.")
                        return None

                # 5. Other terminal/failure states
                if state in ["failed", "shutdown", "rejected"]:
                    logging.warning(f"❌ {service_name} task failed ({state}) on node {node_id}")
                    return None

            # Wait and retry before deadline
            time.sleep(1)

        # After timeout: log all tasks again
        logging.debug(f"🔍 Inspecting {len(tasks)} tasks for service {service_name}")
        for task in tasks:
            logging.debug(
        ...
        logging.warning(f"❌ No running task with valid NodeID found for {service_name} after timeout")
    except APIError as e:
...
def force_update_service(service_name):
    try:
        ...
        try:
            # Use low-level API to provide full spec if using newer SDKs
            if DOCKER_SDK_VERSION >= (4, 4, 0):
            ...
            # This fallback works for most simple updates
...
def main_loop():
    ...
    for anchor_label in dependencies:
        ...
        if not node_id or node_id == "starting":
            logging.warning(f"❌ {full_db_service} is not running or still starting. Skipping.")
            continue
    ...
    for anchor_label, config in dependencies.items():
        ...
        if not db_node or db_node == "starting":
            logging.warning(f"❌ Could not find anchor node for {db_service} or it's still starting. Skipping.")
            continue
        ...
        for dep in dependents:
            ...
            try:
                ...
                if dep_node == "starting":
                    logging.info(f"⏳ {full_dep_service} is still starting. Will not update yet.")
                    continue

                logging.debug(f"🔍 DEBUG: dep_node = {dep_node}, full_dep_service = {full_dep_service}")

                # CASE 1: Service is not running
                if not dep_node:
                    if should_retry(full_dep_service, retry_schedule):
                        logging.warning(f"❌ {full_dep_service} is not running or stuck. Retrying update.")
                        force_update_service(full_dep_service)
                    else:
                        logging.info(f"⏳ Skipping retry for {full_dep_service} (waiting cooldown)")
                    continue

                # CASE 2: Service is running but on the wrong node
                if db_node != dep_node:
                    if should_retry(full_dep_service, retry_schedule):
                        logging.info(f"🔁 {full_dep_service} not co-located with {anchor_label}. Retrying update.")
                        force_update_service(full_dep_service)
                    else:
                        logging.info(f"⏳ Skipping retry for {full_dep_service} (waiting cooldown)")
                else:
                    # CASE 3: Already colocated and running
                    logging.info(f"✅ {full_dep_service} already co-located with {anchor_label}.")
                    logging.debug(f"👌 {full_dep_service} is colocated and stable. No restart required.")
            except Exception as e:
...
def run():
    ...
    if EVENT_MODE:
        ...
        threads.append(threading.Thread(target=event_thread))
    ...
    if POLLING_MODE:
        ...
        threads.append(threading.Thread(target=polling_thread))
